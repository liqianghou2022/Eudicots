...
# usage: python 01.merge-aligment.py -i <input_dir> -o <output.fasta>  (or edit the paths in __main__)
...
def get_all_ids_and_lengths(input_dir):
    ...
    for file_name in os.listdir(input_dir):
        ...
        if os.path.isfile(file_path):
            ...
            if sequences:
                file_lengths[file_name] = len(next(iter(sequences.values())))  # Assume equal length sequences in a file
            else:
    ...
    with open(output_file, 'w') as out_file:
        ...
        for file_name in os.listdir(input_dir):
            ...
            if os.path.isfile(file_path):
                ...
                sequences = read_fasta(file_path)
                seq_length = file_lengths[file_name]  # Sequence length in the current file
                ...
                for seq_id in all_ids:
                    if seq_id in sequences:
                        ...
                        concatenated_sequences[seq_id] += "-" * seq_length  # Fill with '-' when missing
...
if __name__ == "__main__":
    ...
    input_directory = "path_to_your_sequence_files"
    # Output file path
    ...
    print(f"All sequences have been concatenated successfully. Results saved to {output_file_path}")